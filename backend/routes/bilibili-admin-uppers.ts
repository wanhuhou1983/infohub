...
export function createBilibiliAdminUppersRoutes(sql: Sql): Hono {
  ...
  router.patch('/:id/toggle', async (c) => {
    try {
      ...
      // 统一返回 { id, name, enabled } 以匹配其他模块的 toggle 格式
    ...
    try {
      ...
      for (const upper of enabledUppers) {
        ...
        for (const video of list) {
          ...
          const [inserted] = await sql`
              INSERT INTO articles (source_id, title, url, external_id, author, summary, published_at, fetched_at, extra)
              VALUES (${sourceId}, ${title}, ${url}, ${bvid}, ${author}, ${title.slice(0, 150)}, ${pubdate}, NOW(), ${sql.json(extra)})
