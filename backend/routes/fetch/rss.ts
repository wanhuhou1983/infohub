...
const WHISPER_TRANSCRIBE_PREVIEW_SEC = 600; // placeholder
/**
 * RSS 采集路由
 * 
...
async function fetchRssFeed(
  ...
  const results = await Promise.all(items.map((item: any) => limit(async () => {
    try {
      ...
      // 全文抓取（RSS 内容已较完整时跳过 MinerU，加速 30s/篇）
      let fullContent = rssContent;
      if (url && rssContent.length < 500) {
        try {
      ...
      if (insertedRows.length > 0) {
        ...
        if (enclosureUrl && enclosureUrl.match(/\.(mp3|m4a|ogg|wav|aac|flac)$/i)) {
          // Mac: Whisper transcription skipped (will use s1 API)
        }
...
export function createRssRoutes(sql: Sql): Hono {
  ...
  router.post('/rss', async (c) => {
    try {
      ...
      // 调用统一采集函数（含并发加速）
      const { fetched, inserted, translated } = await fetchRssFeed(
        sql, feedUrl, sourceId, sourceName, 0
      );
