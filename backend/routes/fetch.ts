...
export function createFetchRoutes(sql: Sql): Hono {
  ...
  router.post('/penti', async (c) => {
    try {
      ...
      const resp = await fetch(listUrl, { signal: AbortSignal.timeout(30000), headers: { 'User-Agent': 'Mozilla/5.0' } });
      ...
      const artResp = await fetch(base + targetUrl.href, { signal: AbortSignal.timeout(30000), headers: { 'User-Agent': 'Mozilla/5.0' } });
